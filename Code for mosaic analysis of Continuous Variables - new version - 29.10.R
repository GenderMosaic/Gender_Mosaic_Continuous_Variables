...
#Results:
  ...
  # Notice: there are four colors in each mosaic : Blue for Male-typical, Pink for Female-typical, White for neutral and Grey for missing values.
...
Cohens_D_dist = function(Data){
  ...
  for (i in 1:Number_of_Variables){
    ...
    Cohens_D[i]=cohensD(Data_M[,i],Data_F[,i])*sign
  }
...
General_Mosaic = function(Data,CohensD_Min,Percentage=0.33){
  ...
  M_M = levelplot(t(Mosaic_M[nrow(Mosaic_M):1,]),col.regions = colors,asp=2,at=seq(-2, 1, by = 0.75),cut=3,xlab="Variables",
                      ...
                      scales=list(x=list(rot=60,cex=.5,alternating=2,tck=c(0,1)),y=list(tck=c(1,0))))
  #females mosaic
...
results <- General_Mosaic(Data=df,CohensD_Min=0.5,Percentage=0.67) #creating the mosaic
